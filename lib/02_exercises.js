...
[1, 2, 3, 4, 5, 6, 7].forEach(function (times) {
...
range.forEach(function (times) {
  // console.log("#".repeat(times));
});

new_exercise();

var is_fizz = function is_fizz(number) {
  return number % 3 == 0;
};
var is_buzz = function is_buzz(number) {
  return number % 5 == 0;
};

console.log("Test");

var assert = function assert(expected, actual) {
  if (expected === actual) {
    console.log('.');
  } else {
...
var assert_true = function assert_true(actual) {
  assert(true, actual);
};
var assert_false = function assert_false(actual) {
  assert(false, actual);
};
...
var fizz = function fizz(number) {
  return number % 3 == 0 ? "Fizz" : "";
};
var buzz = function buzz(number) {
  return number % 5 == 0 ? "Buzz" : "";
};
var empty = function empty(str, element) {
  return str == "" ? element : "";
};
...
var range = Array.apply(null, { length: board_size * board_size }).map(Number.call, Number).map(board_mapper(board_size)).join('');
